...
from functions import user_pois
...
def knn(ftrain, ftest, out):
        ...
        for user in users_visit.keys():
            ...
            # Si también lo ha visitado el usuario que estamos viendo sumamos su producto
                if poi in users_visit[user].keys():
                    num += int(users_visit[user][poi]) * int(users_visit[user][poi])
    ...
    print(fifty_pois(sort_recomendations(rating), user_pois))
